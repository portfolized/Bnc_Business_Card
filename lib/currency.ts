...
/** Default affiliate commission rate (fraction) — the global "all persons" rate
 *  until an admin changes it. A per-affiliate override can replace it. */
export const AFFILIATE_RATE = 0.1;

/** Default buyer discount rate (fraction) applied when an affiliate code is used
 *  at checkout. 0 = no discount until an admin sets one. */
export const AFFILIATE_DISCOUNT_RATE = 0;
