...
function SocialIcon({ label, children }: { label: string; children: ReactNode }) {
  return (
    <a
      href="#"
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 text-subtext transition-colors hover:border-primary hover:text-primary"
    >
      {children}
    </a>
  );
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-subtext">
              Transform your networking experience with our smart NFC business
              cards. Connect, share, and grow your network seamlessly.
            </p>
            ...
            <ul className="space-y-2">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-sm text-subtext transition-colors hover:text-primary"
                  >
                    {link.label}
                  </a>
                </li>
